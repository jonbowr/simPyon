...
def voltage_optimize(volt_base_dict,electrodes,opt_param_func,
        constraint_func,opt_func = np.nanmax,coms = [],
        generations = 10, siblings = 8, mut_w = .1,n_parts = 10000,
        walk = False,repeat_max = 4):
    np.set_printoptions(precision = 2)
    volt_shift = dict((name,volt_base_dict[name]) for name in volt_base_dict)
    volt_hold = dict((name,volt_base_dict[name]) for name in volt_base_dict)
    volts_gens = []
    ...
    coms.fly(n_parts = n_parts,quiet = True)
    ...
    for i in range(1,generations):
        ...
        tot_volts.append(volts_siblings)
    plt.subplots()
    gens=0
    ...
    # plt.figure(2)
    # plt.plot(scale_fact,de_e)
    return(param_gens,volts_gens,{'params':total_params,'voltages':tot_volts})


def voltage_scrub(volt_base_dict,electrodes,split_num,coms = [], mut_w = .1,n_parts = 10000):
    from ipywidgets import IntProgress
    from IPython.display import display
    volt_shifts = []
    for e in electrodes:
        volt_shifts.append(np.linspace((1-mut_w)*volt_base_dict[e],(1+mut_w)*volt_base_dict[e],split_num))
    
    shift_grid = np.meshgrid(*volt_shifts)
    tot_shifts = []
    for s in shift_grid:
        tot_shifts.append(s.flatten())
    tot_shifts = np.stack(tot_shifts).T
    
    f = IntProgress(min=0, max=len(tot_shifts)) # instantiate the bar
    display(f) # display the bar
    out_d = {'shift':tot_shifts,'volts':[],'data':[]}
    for shift in tot_shifts:
        shift_volts = {nam:volt_base_dict[nam] for nam in volt_base_dict}
        for v_s,nam in zip(shift,electrodes):
            shift_volts[nam]=v_s
        out_d['volts'].append(shift_volts)
        coms.fast_adjust(shift_volts,quiet = True)
        out_d['data'].append(coms.fly(n_parts,quiet = True).data)
        f.value+=1
    return(out_d)
...
def location_chng(shift_gem, out_gem,shift_names,shift_vals):
    with open(shift_gem) as lines:
        ...
        for tag,val in zip((shift_names if type(shift_names) is list else [shift_names]),
                           (shift_vals if type(shift_vals) is list else[shift_vals])):
            n = 0
